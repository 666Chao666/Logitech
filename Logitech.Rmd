...
# manual inspection indicates multiple Null's, yet ... 
sum(is.na(df))

# check data type
sapply(df, class)

# filter with exact match of Null
df$Category3 == 'NULL'

# locate rows with NULL character string
nulls <- df %>%
  filter((df$Category1 == 'NULL' | df$Category2 == 'NULL' |
          df$Category3 == 'NULL' | is.na(.)))
# While the NULL character strings don't necessarily have a direct impact on time series analysis, missing values might indicate potential data quality issues and due to the lack of extra information on the dataset, removing them is a ...
